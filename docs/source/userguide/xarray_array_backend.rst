...
.. code-block:: python
    ...
    # xarray-oriented operation (also accepts arrays)
...
The philosophy is to return the same type of object as the input, ensuring consistency across operations. For example,

.. code-block:: python

    import earthkit.data as ekd
    network = ekh.river_network.load("efas", "5")
    ds = ekd.from_source("file", "data.nc").to_xarray() # xarray dataset
    output = ekh.upstream.sum(network, ds) # xarray dataset returned

    da = ds['main_variable'] # xarray dataarray
    output = ekh.upstream.sum(network, da) # xarray dataarray returned
