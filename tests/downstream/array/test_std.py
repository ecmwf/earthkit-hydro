...
def test_downstream_metric_std(river_network, input_field, mv):
    ...
    output_field = ekh.downstream.array.std(river_network, input_field, node_weights=None, return_type="masked")
    ...
    std_uniform = ekh.downstream.array.std(river_network, uniform_field, node_weights=None, return_type="masked")
    np.testing.assert_allclose(std_uniform, 0, atol=1e-10)

    # Test relationship with variance
    var_field = ekh.downstream.array.var(river_network, input_field, node_weights=None, return_type="masked")
