...
def test_downstream_min_xarray(river_network, input_field, expected, mv):
    """Test downstream min with xarray input."""
    field_da = xr.DataArray(input_field, dims=["node_index"], coords={"node_index": np.arange(len(input_field))})
    result = ekh.downstream.min(river_network, field_da, return_type="masked")
