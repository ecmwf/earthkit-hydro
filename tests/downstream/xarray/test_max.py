...
def test_downstream_max_xarray(river_network, input_field, expected, mv):
    """Test downstream max with xarray input."""
    field_da = xr.DataArray(input_field, dims=["node"])
    result = ekh.downstream.max(river_network, field_da, return_type="masked")
