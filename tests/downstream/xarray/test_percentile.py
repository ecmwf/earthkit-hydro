...
def test_downstream_percentile_xarray_masked(flow_directions, input_field, expected_masked, p):
    rn = make_river_network_with_coords(flow_directions)
    field_xr = field_to_xarray(rn, input_field)

    # masked return type
    result = ekh.downstream.percentile(rn, field_xr, p=p, return_type="masked")
...
def test_downstream_percentile_xarray_gridded(flow_directions, input_field, expected_gridded, p):
    rn = make_river_network_with_coords(flow_directions)
    field_xr = field_to_xarray(rn, input_field)

    # gridded return type
    result = ekh.downstream.percentile(rn, field_xr, p=p, return_type="gridded")
    assert isinstance(result, xr.DataArray)
