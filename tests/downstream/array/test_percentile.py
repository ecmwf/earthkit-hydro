...
def test_downstream_percentile_unweighted(river_network, input_field, expected, p):
    output = ekh.downstream.array.percentile(river_network, input_field, p=p, node_weights=None, return_type="masked")
    ...
    output_2d = ekh.downstream.array.percentile(river_network, input_2d, p=p, node_weights=None).flatten()
