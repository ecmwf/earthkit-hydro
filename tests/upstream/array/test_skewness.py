...
def test_calculate_upstream_metric_skewness(river_network, input_field, flow_downstream, mv, array_backend):
    river_network = river_network.to_device("cpu", array_backend)
    xp = ekh._backends.find.get_array_backend(array_backend)
    output_field = ekh.upstream.array.skewness(
    ...
    np.testing.assert_allclose(output_field, flow_downstream, rtol=1e-5, atol=1e-9, equal_nan=True)
