...
def test_upstream_max_xarray(river_network, input_field, expected, mv):
    """Test upstream max with xarray input."""
    # Convert input to xarray DataArray
    field_da = xr.DataArray(input_field, dims=["node"])

    # Call xarray-wrapped function
    result = ekh.upstream.max(river_network, field_da, return_type="masked")
