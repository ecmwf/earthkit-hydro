...
pub fn calc_weighted_perc_downstream<'py>(
    ...
    let downstream_map: DashMap<i64, (Vec<f64>, Vec<f64>)> = DashMap::new();
...
fn process_level_downstream(
    ...
    downstream_map: &DashMap<i64, (Vec<f64>, Vec<f64>)>,
    ...
    // Clone did's entry (not remove) as multiple uids may receive from the same did.
    did_slice
        .par_iter()
        .zip(uid_slice.par_iter())
        .for_each(|(&did, &uid)| {
            let (did_vals, did_wts) = downstream_map
                .get(&did)
                .map(|entry| entry.clone())
                .unwrap_or_else(|| (vec![field[did as usize]], vec![weights[did as usize]]));

            downstream_map
                .entry(uid)
                .and_modify(|(uid_vals, uid_wts)| {
                    merge_sorted_weighted(uid_vals, &did_vals, uid_wts, &did_wts);
                })
                .or_insert_with(|| {
                    let uid_val = field[uid as usize];
                    let uid_wt = weights[uid as usize];
                    let mut vals = did_vals;
                    let mut wts = did_wts;
                    let pos = match binary_search_f64(&vals, uid_val) {
                        Ok(pos) | Err(pos) => pos,
                    };
                    vals.insert(pos, uid_val);
                    wts.insert(pos, uid_wt);
                    (vals, wts)
                });
        });
    ...
    let pct_results: Vec<(i64, f64)> = uid_slice
        .par_iter()
        .map(|&uid| {
            let entry = downstream_map.get(&uid).unwrap();
...
fn weighted_percentile(sorted_values: &[f64], weights: &[f64], p: f64) -> f64 {
    ...
    for i in 0..n - 1 {
        let next_cum = cum_before + weights[i];
        if target <= next_cum {
