...
def test_xarray_wrapper_matches_array_backend(p, weighted):
    ...
    result = ekh.downstream.percentile(
        river_network,
    ...
    expected = ekh.downstream.array.percentile(river_network, field, p=p, node_weights=weights, return_type="masked")
...
def test_xarray_gridded_return_type():
    river_network = gridded_network(cama_nextxy_1)
    field = make_field(river_network)
    result = ekh.downstream.percentile(river_network, to_dataarray(river_network, field), p=0.5, return_type="gridded")
    masked = ekh.downstream.array.percentile(river_network, field, p=0.5, return_type="masked")
    assert isinstance(result, xr.DataArray)
